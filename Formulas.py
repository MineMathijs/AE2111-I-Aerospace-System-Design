...
def cruiseTTW(mass_fraction, C_D0, mach, aspect_ratio, span_efficiency_factor, wing_loading, cruise_altitude, std_temp=288.15, thetaTbreak=1.6):  # OK
    ...
    if thetaT < thetaTbreak:
        ...
        thrust_lapse = deltaT * \
            (1-(0.43+0.014*5)*math.sqrt(mach) - 3*(thetaT*thetaTbreak)/(1.5+mach))
...
def climbGradientTTW(mass_fraction, climb_gradient, wing_loading,  C_D0, aspect_ratio, span_efficiency_factor, OEI=False, NE=2):
    thrust_lapse = Alpha(C_D0, aspect_ratio, wing_loading,
                         span_efficiency_factor, mass_fraction, 0)

    TTW = (mass_fraction / thrust_lapse) * (climb_gradient + 2 *
                                            math.sqrt(C_D0/(math.pi * aspect_ratio * span_efficiency_factor)))
...
def takeOffFieldLength(wing_loading, TO_field_length, rho,  AR, span_efficiency_factor, h2, k_t=0.85, g=9.80665, OEI=False, NE=2):
    ...
    if OEI:
        TTW = (1.15 * alphaT * math.sqrt(wing_loading/(k_t * g * rho * math.pi *
               AR * span_efficiency_factor * TO_field_length))) + 4 * h2/TO_field_length
    else:
        TTW = (1.15 * alphaT * math.sqrt((NE/(NE-1))*wing_loading/(k_t * g * rho * math.pi *
               AR * span_efficiency_factor * TO_field_length))) + 4 * h2/TO_field_length * (NE/(NE-1))
    return TTW


def climbRateTTW(climb_rate_requirement, mass_fraction, C_D0, aspect_ratio, wing_loading, span_efficiency_factor):
    thrust_lapse = Alpha(C_D0, aspect_ratio, wing_loading,
                         span_efficiency_factor, mass_fraction,  climb_altitude=7400)
